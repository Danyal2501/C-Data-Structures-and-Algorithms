 ...
 *
 * Student Name: Rana Danyal
 * Student Number: 1006329206
 * UTORid: ranadany
 ...
 *
 * Sign here with your name: Danyal Rana
 *
 *
 */
...
ReviewNode *newMovieReviewNode(char *title, char *studio, int year, double BO_total, int score)
{
	...
	if (year > 2999 || year < 1920 || score > 100 || score < 0){
		return NULL;
	}
	
	ReviewNode *newReview = NULL;
	
	newReview = (ReviewNode*)calloc(1, sizeof(ReviewNode));
	
	if (newReview == NULL){
		return NULL;
	}
	
	strcpy(newReview -> review.movie_title, title);
	strcpy(newReview -> review.movie_studio, studio);
	newReview -> review.year = year;
	newReview -> review.BO_total = BO_total;
	newReview -> review.score = score;
	newReview -> review.cast = NULL;
	newReview -> next = NULL;

    return newReview;  // Remove this before you implement your solution!
}
...
ReviewNode *findMovieReview(char *title, char *studio, int year, ReviewNode *head)
{
	...
	ReviewNode *check = head;

	while (check != NULL){

		if (strcmp(check->review.movie_title,title) == 0 && strcmp(check->review.movie_studio,studio)==0 && check->review.year == year){
			return check;
		}
                              ...
                              ReviewNode *head)
{
	...
	ReviewNode *NewReview = head;

	if (findMovieReview(title,studio,year,head) == NULL){
		NewReview = newMovieReviewNode(title,studio,year,BO_total,score);
                       ...
                       ReviewNode *head)
{
    ...
    /**********  TODO: Complete this function *********************************/
    /***************************************************************************/
	ReviewNode *OldReview = findMovieReview(title,studio,year,head);
	
	if (OldReview != NULL){
		OldReview -> review.BO_total = BO_total;
		OldReview -> review.score = score;
	}
	else{
...
ReviewNode *deleteMovieReview(char *title, char *studio, int year, ReviewNode *head)
{
	...
	ReviewNode *pre = head;
	ReviewNode *tr = head -> next;
	
	if (findMovieReview(title,studio,year,head) == head){
		free (head);
		head = tr;
	}
	
	else{
		while (tr != findMovieReview(title,studio,year,head)){	
			pre = tr;
			tr = tr -> next;		
		}
		if (tr == findMovieReview(title,studio,year,head)){
				pre -> next = tr -> next;
				free(tr);
				return head;
			}
...
double printMovieReviews(ReviewNode *head)
{
	...
	while (current!=NULL){
		printf("%s\n%s\n%i\n%f\n%i\n*******************\n",current -> review.movie_title, current -> review.movie_studio, current -> review.year, current -> review.BO_total, current -> review.score);
		total += current -> review.BO_total;
...
double queryReviewsByScore(int min_score, ReviewNode *head)
{
	...
	while (current!=NULL){
		if (current -> review.score >= min_score){
			printf("%s\n%s\n%i\n%f\n%i\n*******************\n",current -> review.movie_title, current -> review.movie_studio, current -> review.year, current -> review.BO_total, current -> review.score);
			total += current -> review.BO_total;
		}
		current = current -> next;
...
ReviewNode *sortReviewsByTitle(ReviewNode *head)
{
	...
	int headval = countReviews(head);
	
	char temptitle[1024]=" ";
	char tempstudio[1024]= " ";
	int tempyear = 0;
	float tempBO_total = 0;
	int tempscore = 0;
	...
	for (int i=0;i<headval-1;i++){
		for (int j=0;j<headval-i-1;j++){

			if (strcmp(pre->review.movie_title,tr->review.movie_title)>0){
								
				strcpy(temptitle, tr->review.movie_title);
				strcpy(tempstudio, tr->review.movie_studio);
				tempyear = tr -> review.year;
				tempBO_total = tr -> review.BO_total;
				tempscore = tr -> review.score;
				
				head=deleteMovieReview(tr->review.movie_title,tr->review.movie_studio,tr -> review.year,head);
				head=insertMovieReview(temptitle,tempstudio,tempyear,tempBO_total,tempscore,head);
			}
...
void insertCastMember(char *title, char *studio, int year, ReviewNode *head, char *name)
{
    ...
    /**********  TODO: Complete this function *********************************/
    /***************************************************************************/
	CastList *newNode = NULL;
	
	ReviewNode *movie = findMovieReview(title,studio,year,head);
	
	if (movie != NULL){	
		newNode = (CastList*)calloc(1,sizeof(CastList));
		strcpy(newNode ->name, name);
		newNode ->next = movie -> review.cast;
		movie -> review.cast = newNode;
	}
...
int countNames(MovieReview *movie, char *name)
{
	...
	CastList *currentname = movie -> cast;
	

	while (currentname != NULL){
		for (int i=0;i<strlen(currentname->name); i++){
			if (currentname->name[i] == name[j]){
				j++;
			}
			else{
				j=0;
