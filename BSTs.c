...
BST_Node *newBST_Node(double freq, int bar, double index)
{
     ...
     ****/
	 ...
	 newNode = (BST_Node*) calloc(1, sizeof(BST_Node));
	 
	 newNode->key = (10.0*bar)+index;
	 newNode->freq = freq;
	 newNode->bar = bar;
	 newNode->index = index;
...
BST_Node *find_successor(BST_Node *right_child_node)
{
     ...
     ****/
	 ...
	 BST_Node* current = right_child_node;
     ...
     ****/
	 ...
	 if(root == delete){
		 if (root->left == NULL && root->right == NULL){
			 ...
			 root -> left = BST_delete(root->left,temp->bar,temp->index);
     ...
     ****/
	 ...
	 for (int i=0; i<100; i++){
		 if (note_freq[i] == root->freq){
			 strcpy(name, note_names[i]);
		 }
	 }
	 
	 
	 if (strcmp(name, note_src) == 0){
		 
		 for (int i=0; i<100;i++){
			 if (strcmp(note_names[i],note_dst) == 0){
				 root->freq = note_freq[i];
			 }
		 }
	 }
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
     ...
     ****/
	 ...
	 if (valid != 0){
		 while (BST_search(root,root->bar,root->index + time_shift) != NULL && root->index+time_shift < 1){
